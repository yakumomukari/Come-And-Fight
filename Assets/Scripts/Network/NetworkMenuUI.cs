...
namespace ComeAndFight.Networking
{
    public sealed class NetworkMenuUI : MonoBehaviour
    {
        ...
        DuelNetworkController controller;
        ...
        void Start()
        {
            ...
            controller = FindObjectOfType<DuelNetworkController>();
        ...
        void Update()
        {
            ...
            if (!controller) controller = FindObjectOfType<DuelNetworkController>();
            if (!controller || !statusText) return;

            if (controller.Phase == NetworkTurnPhase.WaitingForPlayers && bootstrap && bootstrap.Manager.IsListening)
            {
                if (bootstrap.Manager.IsHost && Time.unscaledTime < copiedStatusUntil) return;
                statusText.text = bootstrap.Manager.IsHost ? WaitingForOpponentText() : Application.isMobilePlatform ? "正在通过 Relay 连接房主…" : "正在通过 Steam 连接房主…";
            }
            else if (controller.Phase == NetworkTurnPhase.Paused)
            {
            ...
            else if (controller.Phase == NetworkTurnPhase.Choosing || controller.Phase == NetworkTurnPhase.Resolving || controller.Phase == NetworkTurnPhase.GameOver)
            {
        ...
        public void ShowOnlinePanel()
        {
            modePanel.SetActive(false);
            onlinePanel.SetActive(true);
            if (!bootstrap || !bootstrap.Manager.IsListening) SetCreateButtonCaption(Application.isMobilePlatform ? "创建 Relay 房间" : "创建房间（Host）");
            statusText.text = Application.isMobilePlatform ? "输入房主分享的 Relay 房间码" : "输入房主显示的 17 位 Steam ID，或 Ctrl+V 粘贴";
            if (duelGame) duelGame.InputLocked = true;
        ...
        {
            if (!bootstrap) return;
            if (bootstrap.Manager.IsListening && bootstrap.Manager.IsHost)
            {
                CopyHostSteamId();
                return;
            }
            SetConnectionButtons(false);
            statusText.text = Application.isMobilePlatform ? "正在连接 Unity 服务并创建房间…" : "正在创建 Steam 房间…";
            ...
            else
            {
                ...
                SetCreateButtonCaption(Application.isMobilePlatform ? "复制 Relay 房间码" : "复制房主 Steam ID");
            }
        }
        ...
        public void CopyHostSteamId()
        {
            ...
            statusText.text = (Application.isMobilePlatform ? "已复制 Relay 房间码：" : "已复制房主 Steam ID：") + bootstrap.LocalAddressSummary;
        ...
        public async void JoinRoom()
        {
            ...
            SetConnectionButtons(false);
            statusText.text = Application.isMobilePlatform ? "正在验证房间码并连接 Relay…" : "正在连接 Steam 房主…";
        ...
        public void CancelOnline()
        {
            DisconnectIfNeeded();
            SetConnectionButtons(true);
            SetCreateButtonCaption(Application.isMobilePlatform ? "创建 Relay 房间" : "创建房间（Host）");
            ShowModeSelection();
        }
        ...
        void ConfigurePlatformUi()
        {
            ...
            if (label) label.text = Application.isMobilePlatform ? "Android Relay 对战" : "Steam 在线对战";
            if (descriptionText) descriptionText.text = Application.isMobilePlatform
                ? "Android Relay 联机 · 创建或输入房间码\n需要互联网连接"
                : "Steam P2P / Relay · 输入房主显示的 Steam ID\n双方需启动 Steam 并登录不同账号";
            if (Application.isMobilePlatform)
            {
                SetCreateButtonCaption("创建 Relay 房间");
                SetButtonCaption(joinButton, "通过房间码加入");
                if (addressInput && addressInput.placeholder is Text placeholder) placeholder.text = "Relay 房间码";
            }
        }
        ...
        {
            return Application.isMobilePlatform ? "等待对手 · Relay 房间码：" + bootstrap.LocalAddressSummary : "等待对手 · 房主 Steam ID：" + bootstrap.LocalAddressSummary;
        }

        void OnGUI()
        {
            if (Application.isMobilePlatform || !onlinePanel || !onlinePanel.activeInHierarchy || !addressInput || bootstrap && bootstrap.Manager.IsListening) return;
